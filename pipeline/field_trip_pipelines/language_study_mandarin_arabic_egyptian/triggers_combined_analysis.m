...
% Matlab names the channels incremented by 1, so 224 on KIT becomes 225 in
% MATLAB
% Channels are named simply '225', '226', ..., '232'
...
% dataTrigger.trial{1} is now an 8 x N matrix (8 channels, N time samples)
...
% In the matrix we need to compute the code


% If the wiring is reversed (i.e., channel 225 is the most significant bit),
% you would flip the exponents:
% bitWeights = 2.^(7:-1:0)'; 

%% -- 3) Detect trigger onsets by looking at code changes
diffCode  = [0, diff(triggerCodePerSample)];
changeIdx = find(diffCode ~= 0);

% Trigger codes at those change points
onsetCodes = triggerCodePerSample(changeIdx);

% Times (seconds) for each trigger onset
onsetTimes = changeIdx / dataTrigger.fsample;

%% -- 4) (Optional) Build a FieldTrip-like event structure
clear event
...
disp('----------------------------------');
disp('Observed counts of each trigger code:');
disp(observedTable);
disp('----------------------------------');
