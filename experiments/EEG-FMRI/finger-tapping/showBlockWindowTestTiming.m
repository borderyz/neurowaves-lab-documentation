...
%         end
        ...
        for frame = 1 : numFrames
            ...
            totalTime = GetSecs - startTimeGetSecs;
            disp(['total time first loop', int2str(totalTime)]);

            if totalTime>parameters.blockDuration
                break;
                disp('break happened in numframe loop');
            end
        ...
        for leftOutWaitFrame = 1:leftoutFrames
                ...
                totalTime = GetSecs - startTimeGetSecs;
                disp(['total time second loop', int2str(totalTime)]);
                if totalTime>parameters.blockDuration
                    break;
                    disp('break happened in leftoutwaitframe');
                end
